...
- **One-to-two-line `info` context above top-level module prompts.** For each module's entry-point `ask_yesno`, precede it with one or two `info "..."` lines explaining what the step actually does and how it relates to neighbouring steps (example: `25-firewall` clarifies it's a packet filter, not intrusion detection). Keep it terse — these are operator hints, not a manpage. Skip it when the prompt label and default are already self-explanatory (e.g. `"Install Tailscale VPN?"`).

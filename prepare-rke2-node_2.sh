...
#   enp7s0 — private interface (UFW: allow all, trusted inter-node traffic)
#
...
if [[ -n "${SSH_SERVICE}" ]] && sshd -t 2>/dev/null; then
    systemctl reload "${SSH_SERVICE}"
    log "SSH hardened (key-only, no root, max 3 retries) [service: ${SSH_SERVICE}]"
else
